"use strict";
// Modified from
// https://github.com/microlinkhq/spotify-url-info/blob/master/index.js
...
function getData(url) {
  ...
  if (!parsedURL.type) {
    return Promise.reject(new TypeError(`Failed to parse '${url}' as Spotify URL`));
  }
...
// ===========
const getAllTracksPlaylist = async (url) => {
  let data = await getData(url);
  const allTracks = data.tracks.items.map((t) => t.track);

  while (data.tracks.next) {
    data = await getData(data.tracks.next);
    allTracks = allTracks.concat(data.tracks.items.map((t) => t.track));
  }
...
function sanityCheck(data) {
  ...
  if (!SUPPORTED_TYPES.includes(data.type)) {
    return Promise.reject(new Error(`Not an ${SUPPORTED_TYPES.join(", ")}. Only these types can be parsed`));
  }
  return Promise.resolve(data);
}
...
module.exports.getAllTracksPlaylist = getAllTracksPlaylist;
